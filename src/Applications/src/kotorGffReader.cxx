...
int main(int argc, char** argv){
  ...
  clp.addOption("filePath", {"-f", "--file-path"}, "Path to gff file");
  ...
  GenericFileFormat gffFile{};

  LogInfo << "Reading file: " << clp.getOptionVal<std::string>("filePath") << std::endl;
  gffFile.read( clp.getOptionVal<std::string>("filePath") );

  if( clp.isOptionTriggered("verbose") ){
    LogDebug << gffFile.getSummary() << std::endl;
  }
